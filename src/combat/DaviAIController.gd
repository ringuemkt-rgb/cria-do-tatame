...
func record_player_action(action_id: String) -> void:
	...
	var technique: Dictionary = _get_technique(action_id)
	var family: String = str(technique.get("family", technique.get("familia", "geral")))
	seen_player_families[family] = int(seen_player_families.get(family, 0)) + 1

func _get_technique(technique_id: String) -> Dictionary:
	var registry: Node = _registry()
	if registry == null or not registry.has_method("get_technique"):
		return {}
	var value = registry.call("get_technique", technique_id)
...
func choose_technique(combat_manager: Node) -> Dictionary:
	...
	if affordable.is_empty():
		return {}
	var actor_state: String = str(combat_manager.call("get_actor_state_name", rival_id))
	var player_id: String = str(combat_manager.get("player_id"))
...
func _score_technique(
	...
	var gas_ratio: float = float(rival_resources.get("gas", 0)) / 100.0
	var focus_ratio: float = float(rival_resources.get("focus", 0)) / 100.0
...
func _risk_penalty(technique: Dictionary) -> float:
	...
	var configured_risk: float = float(profile.get("tuning", {}).get("risk", 0.35))
