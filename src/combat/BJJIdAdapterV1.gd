...
func normalize_technique(value: String, id_namespace: String = "auto") -> Dictionary:
	if not is_ready():
	...
	if id_namespace in ["auto", "canonical", "card"] and canonical_techniques.has(source):
		return {
			"ok": true,
			"canonical": source,
			"source": source,
			"source_namespace": "canonical" if id_namespace != "card" else "card",
			"mapped": id_namespace == "card"
		}

	if id_namespace in ["auto", "legacy"] and legacy_action_to_canonical.has(source):
		return {
			"ok": true,
...
func normalize_position(value: String, id_namespace: String = "auto") -> Dictionary:
	if not is_ready():
		return {"ok": false, "error": "adapter_not_ready"}
	var source := value.strip_edges()
	if source == "":
	...
	if id_namespace in ["auto", "canonical"] and canonical_positions.has(source):
		return {
			"ok": true,
			"canonical": source,
			"source": source,
	...
	if id_namespace in ["auto", "legacy"] and legacy_state_to_canonical.has(source):
		return {
			"ok": true,
