...
class FourCriticalTargetsV1Tests(unittest.TestCase):
    ...
    def test_existing_external_registry_remains_the_single_authority(self):
        registry_path = ROOT / "data/production/external_tool_registry_v1.json"
        self.assertTrue(registry_path.exists())
        registry = json.loads(registry_path.read_text(encoding="utf-8"))
        ids = {row["id"] for row in registry["sources"]}
        self.assertIn("wolfcha", ids)
        self.assertIn("sprite_animator", ids)
        self.assertIn("pixelsrpg_forge", ids)
        self.assertIn("qwen_2512_pixel_art_lora", ids)
        self.assertFalse((ROOT / "data/production/tool_registry_v1.json").exists())
