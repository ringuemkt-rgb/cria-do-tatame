...
func _run() -> void:
	...
	_assert(str(initial_factions.get("ALE", {}).get("name", "")) == "Os Aleluiado", "Nome canonico de ALE incorreto")
	...
	_assert(str(migrated_director.get("factions", {}).get("ALE", {}).get("name", "")) == "Os Aleluiado", "Mapper nao corrigiu display de ALE")
